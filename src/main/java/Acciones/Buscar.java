...
public class Buscar {    
    ...
    public String [] verproveedor2(int rfcp){
        String sql = "select * from proveedor where id_proveedor= '"+rfcp+"' ;";      
        String []x = new String[4];
        try{
           st=conexion.createStatement();   
           res=st.executeQuery(sql);
    ...
    public String [] verprod(String no){
     ...
     public String [] veridprodu(){
        ...
        try{
           ...
           filas = 0;
           while(res.next()){
               x[filas]=res.getString("id_producto");     
               filas++;
           }  
        }catch(Exception e){
           JOptionPane.showMessageDialog(null,e.getMessage());
